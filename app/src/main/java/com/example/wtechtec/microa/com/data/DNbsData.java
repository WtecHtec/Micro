...
public class DNbsData {
    ...
    public  String GetDIdMmess(String userid){
        ...
        HttpURLConnection conn   = null;
        try {
            path = "http://microa.duapp.com/DNbsDid.jsp?d_id="+ URLEncoder.encode(userid,"utf-8");
            URL url=new URL(path);
            conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            result=getStreamAsString(conn.getInputStream(),"utf-8");
    ...
    public  String GetIdMmess(String userid){
        ...
        HttpURLConnection conn   = null;
        try {
            path = "http://microa.duapp.com/DNbsId.jsp?userid="+ URLEncoder.encode(userid,"utf-8");
            URL url=new URL(path);
            conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            result=getStreamAsString(conn.getInputStream(),"utf-8");
    ...
    public  String GetMmess(String xx){
        ...
        HttpURLConnection conn   = null;
        try {
            path = "http://microa.duapp.com/DNbsSee.jsp?xx="+URLEncoder.encode(xx,"utf-8")+"";
            URL url=new URL(path);
            conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            result=getStreamAsString(conn.getInputStream(),"utf-8");
